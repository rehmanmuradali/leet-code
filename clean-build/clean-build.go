...
func fileSearch(root string, filename string, fileChan chan<- string) {
	...
	for _, file := range files {
		...
		if file.IsDir() && strings.HasSuffix(file.Name(), "build") {
			log.Println("Deleting: " + filepath.Join(root, file.Name()))
...
func main() {
	...
	go fileSearch("/Users/rehmanmuradali/code/bayzat-be-kt", "main.go", fileChan)
